...
h5ad_path = '/home/usr/sc2Flow_alb/datasets/mcfarland_2020_pre.h5ad'
...
for split_id in [4]:
    ...
    checkpoint_save_path = f'./checkpoints/mcfarland/{split_key}'
    result_save_path = f'./results/mcfarland/{split_key}/'
...
#%%
        ...
        existing_indices = original_adata.obs.index.tolist()
        rows_to_duplicate = existing_indices[:3]

        duplicate_obs = original_adata.obs.loc[rows_to_duplicate].copy()
        new_row_names = ['duplicate_' + str(i) for i in range(len(rows_to_duplicate))]
        duplicate_obs.index = new_row_names

        import anndata as ad

        duplicate_adata = original_adata[rows_to_duplicate].copy()
        duplicate_adata.obs.index = new_row_names

        adata_temp = ad.concat([original_adata, duplicate_adata])
        adata_temp.X = 0
