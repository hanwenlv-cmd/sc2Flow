    ...
    def on_train_end(self):
        ...
        if self.config["training"].get("run_eval_r2", False):
            ...
            if self.logger is not None:
                self.logger.experiment.log({"final_train_R2_mean": _train_eval[0]})
                self.logger.experiment.log({"final_test_R2_mean": _test_eval[0]})
                self.logger.experiment.log({"final_ood_R2_mean": _ood_eval[0]})
            else:
                # if don't have logger, print
                print(f"Final Train R2 Mean: {_train_eval[0]}")
                print(f"Final Test R2 Mean: {_test_eval[0]}")
                print(f"Final OOD R2 Mean: {_ood_eval[0]}")
        self.train()
